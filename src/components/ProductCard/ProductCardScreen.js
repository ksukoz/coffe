...
class ProductCardScreen extends Component {
	...
	// fetchData() {
	// 	fetch('http://kawaapi.gumione.pro/api/catalog/item/' + `${this.props.navigation.getParam('productId', '0')}`)
				...
				<Content>
					...
					>
						...
						>
							...
							<View style={styles.phoneNumber} onPress={() => Linking.openURL(`tel:+380994556565`)}>
								<Image
							...
							<View style={styles.phoneNumber} onPress={() => Linking.openURL(`tel:+380674556565`)}>
								<Image
							...
							<View style={styles.phoneNumber} onPress={() => Linking.openURL(`tel:+380934556565`)}>
								<Image
...
const mapDispatchToProps = (dispatch) => ({ getProduct: (id) => dispatch(getProduct(id)) });
