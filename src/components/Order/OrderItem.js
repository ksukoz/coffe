...
class OrderItem extends PureComponent {
	...
	render() {
		...
		const filteredCart = cart && product ? cart.filter((cartItem) => cartItem.id === item.id) : [];
					...
					>
						...
						<Text style={[ styles.productSort, {} ]}>
							...
							{item.pid > 7 ? (
								...
								`, ${item.sort} ${item.arabic_percent ? item.arabic_percent + '%' : ''}`
							)}
						</Text>
						<Text style={styles.productRoast}>{item.pid > 7 ? '' : `Обжарка ${item.roast}`}</Text>
								...
								<TouchableOpacity
									...
									onPress={() =>
										product && filteredCart[0]
											...
											: item.qty > 1 ? this.props.updateCart(item.id, +item.qty - 1) : false}
									activeOpacity={0.9}
								>
									<Icon
										style={{
