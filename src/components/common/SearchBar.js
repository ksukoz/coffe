...
import { getAutocomplete, clearAutocomplete, clearSearchedProducts } from '../../store/actions/catalogActions';
import { searchFocused, getSearch, setSearch, clearAlphabet } from '../../store/actions/commonActions';
...
class SearchBar extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		...
		if (prevProps.navigation.getParam('search') !== this.props.navigation.getParam('search')) {
			this.setState({
	...
	handleSearchInput = (text) => {
		this.setState({ search: text }, () => {
			if (text.length > 1) {
				this.setState({ search: text }, () => {
					this.props.getSearch(text, this.props.navigation.getParam('categoryId', '0'));
				});
			} else {
			...
			>
				...
				>
					{this.state.focus ? (
						...
						<View style={{ flexDirection: 'row', alignItems: 'center' }}>
							...
							<Icon
								...
								onPress={() => {
									this.props.searchFocused();
									TextInputState.focusTextInput(findNodeHandle(this.refs.inputB));
								}}
							/>
						</View>
					)}
					<Input
						...
						onFocus={() => {
							this.state.focus ? '' : this.setState({ focus: true }, () => this.props.searchFocused());
						}}
...
const mapDispatchToProps = (dispatch) => ({
	getAutocomplete: (value, category, page, type) => dispatch(getAutocomplete(value, category, page, type)),
