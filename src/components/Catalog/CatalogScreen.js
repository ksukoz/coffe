...
import { getProductsParams, clearProduct } from '../../store/actions/catalogActions';
...
class CatalogScreen extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (prevProps.navigation.state.params.letter !== this.props.navigation.state.params.letter) {
			this.setState(
				{ page: 0 },
	...
	handleEnd = () => {
		if (!this.props.navigation.state.params.letter) {
			this.props.getProductsParams(this.props.navigation.getParam('categoryId', '0'), this.state.page);
		} else {
			this.props.getProductsParams(
				this.props.navigation.getParam('categoryId', '0'),
				this.state.page,
	...
	render() {
		...
		let notFound;
		if (this.props.products.length === 0 && !this.state.loading && this.state.end) {
			notFound = (
							...
							onEndReached={() =>
								this.props.fetch
									...
									: !this.state.end
										...
										: false}
							ListFooterComponent={() =>
								!this.state.end ? <ActivityIndicator color="#89a6aa" size="large" animating /> : null}
...
const mapDispatchToProps = (dispatch) => ({
	...
	getProductsParams: (category, page, letter) => dispatch(getProductsParams(category, page, letter)),
